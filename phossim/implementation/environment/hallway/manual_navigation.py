...
import utils
from synashvins.phosphene_simulators.gaussian_convolution import \
    PhospheneSimulator


# Create window to display the frames
class Window:
    def __init__(self, windowname='Hallway', size=(1200, 1200)):
        self._size = size
...
class Simulator:
    def __init__(self, low=(26, 26), high=(60, 60), **kwargs):
        self.low_res = PhospheneSimulator(phosphene_resolution=low, **kwargs)
        self.high_res = PhospheneSimulator(phosphene_resolution=high, **kwargs)
        self.sim_mode = 0
...
def main():
    ...
    size = 128
    environment = utils.Environment(ip=ip, port=port,
                                    size=size)
    assert (environment.client is not None), \
        "No server found! Please start Unity environment first"
